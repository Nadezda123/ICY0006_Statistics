...
data <- lm(insurance$charges ~ insurance$age)
summary(data)
```
...
sigma(data)/mean(insurance$charges)
```
Corresponding to 87% error rate
...
In our plot, with age and bmi predicator variables, the adjusted R2 = 0,12, meaning that "12% of the variance in the measure of charges can be predicted by age and bmi. In comparison, this model is better than our simple liner regression model with only age, which had an adjusted R2 of 0.08872.
...
```
Corresponding to 85% error rate, but still better than in the simple model, where RSE was 87% error rate.
