...
table(insurance$sex)
```
Now we can visualize charges distributions based on gender.
...
After calculating the above probabilities of the winning in the US Powerball lottery, I think everyone can see that the chance to win the main price is so small that it is close to impossible. Very nice exercise! Now I can advice my friends not to buy the lottery tickets:-)
...
train <- insurance_2[trainingRowIndex, ] #model training data
test <- insurance_2[-trainingRowIndex, ]    # test data
...
Also with summary function we can see the values for particular column. We can easily compare them between the training and the testing sets.
...
```{r}
#we create the boxplots to see our training data
par(mfrow = c(2,4))
hist(train$charges, col = "Orange", main = "Distribution of charges", xlab="Charges")
hist(train$age, col = "Green", main = "Distribution of age", xlab="Age")
hist(train$bmi, col = "Yellow", main = "Distribution of BMI", xlab="BMI")
barplot(height = table(train$children),col = "Red",  main = "Distribution of children", xlab="No.of children")
barplot(height = table(train$smoker), col="Blue", main = "Distribution of Smoking", xlab = "Smoker")
barplot(height = table(train$sex), col="Purple", main = "Distribution of Sex", xlab = "Sex")
barplot(height = table(train$region), col="Lightblue", main = "Distribution of Region",xlab='Region')
```
...
model5 <- lm(charges ~ age + smoker, data = train)
chargesPred <- predict(model5, test)
summary(model5)

#calculate information criterion and balance between a good fit and over complexity in a model
AIC(model5)
BIC(model5)
...
intercepts <- c(coef(model5)["(Intercept)"], coef(model5)["(Intercept)"] + 
coef(model5)["smokeryes"])
lines.df <- data.frame(intercepts = intercepts,
slopes  = rep(coef(model5)["age"], 2),
smoker = levels(train$smoker))
qplot(x = age, y = charges, color = smoker, data = train) + 
geom_abline(aes(intercept = intercepts, slope = slopes, color =smoker),
data = lines.df) + scale_y_continuous(breaks = seq(0,65000,5000))

#The error rate can be estimated by dividing the RSE by the mean outcome variable
sigma(model5)/mean(train$charges)

```
Based on the data above for the training model, it is very similar to our original model. So explanation above applies.


```{r}
# to check the error term
mean(model5$residuals)
```
Our model has mean almost 0
...
We can see that fitted values increase and linearity seems to hold reasonably well, as the red line is close to the dashed line, with slightly curved pattern. We can also note heteroskedasticity. Finally, points 243, 1302, and 578 may be outliers, with large residual values.
...
```{r}
# making actuals_predicteds dataframe
actuals_preds <- data.frame(cbind(actuals=test$charges, predicteds=chargesPred))
...
We can compute all the error metrics in one shot
```{r}
