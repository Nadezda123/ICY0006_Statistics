...
Now we can visualize charges distribution based on Age.
...
```
Based on this plot we can see that age is very strong predicator in our set, but we will have to test other variables to see a full picture. Also based on this plot we can see that there are other 2 layers exist (dots on 20k and on 40k), so it means that there are 2 more variables that have valuable correlation  with charges.
...
```{r}
#we can also create another version of complete Scatter plot matrix (excluding charges)
library(psych)
...
So I will split our dataset into a 80/20 sample (training/
test). 
By calculating accuracy measures (like min_max accuracy) and error rates (MAPE or MSE), we will see the prediction accuracy of the model.
...
```{r}
plot(x = trainingData$age, y = trainingData$charges,
     xlab = "Age",
     ylab = "Charges",
     xlim = c(15, 70),
     ylim = c(1000,15000),
...
abline(lm(trainingData$charges ~ trainingData$age, data = trainingData), col="blue")
linearReg <- lm(trainingData$age ~ trainingData$charges, data = trainingData)
print(linearReg)
```
...
Also I found that we can compute all the error metrics in one shot
```{r}
