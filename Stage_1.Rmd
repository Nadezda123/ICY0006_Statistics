...
• What kind of variables you have (numerical/categorical etc)?

Describe the variables (the meaning of those).
...
Describe  what you can see from those graphs
...
Summary
Age, smoking and bmi variables have the highest impact on the medical charges.
...
Multiple R-squared value in the first model (model5) is 0.7465 and the second model (model6) is 0.7534 , so it is pretty good improvement.
In our prediction matrix we can see that in the first model it was 0.8768414 (approx 88%) which is much higher than in the second model (model6), where the value is 0.8665147 (approx 87%).
F-statistic that used to estimate the significance of a regression model as a whole in our second model (model6) is much higher 537.8 than in the first model (model5) with value 257.7
...
MAPE express accuracy as a percentage of the error and in our models it is 4% in the first model and 3.8% in the second.
...
RMSE showing us that on average, our predictions varied from observed values by an absolute measure 5954 in the second model and 5264 in the first model.
